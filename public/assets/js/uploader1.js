        ...
        (function( $ ){
          ...
          $(function() {
            ...
            $queue = $( '<ul class="filelist"></ul>' )
                .appendTo( $wrap1.find( '.queueList' ) ),

            // 状态栏，包括进度和控制按钮
            $statusBar = $wrap1.find( '.statusBar' ),

            // 文件总体选择信息。
            $info = $statusBar.find( '.info' ),

            // 上传按钮
            $upload = $wrap1.find( '.uploadBtn' ),

            // 没选择文件之前的内容。
            $placeHolder = $wrap1.find( '.placeholder' ),
        ...
        if ( !WebUploader.Uploader.support('flash') && WebUploader.browser.ie ) {
            ...
            // 压根就没有安转。
                ...
                $wrap1.html('<a href="http://www.adobe.com/go/getflashplayer" target="_blank" border="0"><img alt="get flash player" src="http://www.adobe.com/macromedia/style_guide/images/160x41_Get_Flash_Player.jpg" /></a>');
            }
        ...
        function addFile( file ) {
            ...
            $btns.on( 'click', 'span', function() {
                ...
                if ( supportTransition ) {
                    ...
                    //         $wrap1.css( 'filter', "progid:DXImageTransform.Microsoft.Matrix(M11=" + cos + ",M12=" + (-sin) + ",M21=" + sin + ",M22=" + cos + ",SizingMethod='auto expand')");
                    //     }
                    // });
                }
        ...
        uploader1.onBeforeFileQueued = function( file ) {
            ...
            if (length > 0) {
                for (var i = 0; i < length; i++) {
                    removeFile(files[i])
                    uploader1.removeFile(files[i], true)
                }
            }
        };
        ...
        uploader1.onUploadSuccess = function( file, response ) {
            if (response.error_code == 0) {
                $('input[name="qrcode"]').val(response.result)
            }
        }
